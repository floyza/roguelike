...
template <typename Iter> Iter random_element(Iter begin, Iter end) {
...
template <> struct hash<TCOD_key_t> {
  size_t operator()(const TCOD_key_t &val) const {
    std::size_t hash = 0;
    hash_combine(hash, val.vk, val.c, val.pressed, val.lalt, val.lctrl,
                 val.ralt, val.rctrl, val.shift);
    return hash;
  }
};
template <> struct equal_to<TCOD_key_t> {
  size_t operator()(const TCOD_key_t &v1, const TCOD_key_t &v2) const {
    return v1.vk == v2.vk && v1.c == v2.c && v1.pressed == v2.pressed &&
           v1.lalt == v2.lalt && v1.lctrl == v2.lctrl && v1.ralt == v2.ralt &&
           v1.rctrl == v2.rctrl && v1.shift == v2.shift;
  }
};
} // namespace std
