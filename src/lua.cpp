...
void Lua_manager::init() {
  ...
  lua_state.new_enum<Trigger>("Trigger", {{"DAM_MOD", Trigger::DAM_MOD},
  ...
  lua_state.new_usertype<TCODColor>(
      "color", sol::constructors<TCODColor(), TCODColor(int, int, int)>(), "r",
      &TCODColor::r, "g", &TCODColor::g, "b", &TCODColor::b);
  lua_state.new_usertype<Pos>("Pos", sol::constructors<Pos(), Pos(int, int)>(),
                              "x", &Pos::x, "y", &Pos::y);
  lua_state.new_usertype<Creature>(
      "Creature", sol::no_constructor, "hp", sol::property(&Creature::get_hp),
      "attack", sol::property(&Creature::get_attack), "do_turn",
      &Creature::do_turn, "die", &Creature::die, "pos", &Creature::pos);
  lua_state.new_usertype<Player>(
      "Player",
      sol::constructors<Player(char, const TCODColor &, int, int,
                               const Pos &)>(),
      "hp", sol::property(&Creature::get_hp), "attack",
      sol::property(&Creature::get_attack),
      // "do_attack", sol::overload(&Player::do_attack,
      // 														[](Player
      // &self, Creature &target){
      // self.do_attack(target);}),
      "do_attack",
      [](Player &self, Creature &target, sol::optional<bool> triggers) {
      ...
      "do_turn", &Player::do_turn, "do_move", &Player::do_move, "aquire_item",
      sol::overload(
          static_cast<void (Player::*)(int)>(&Player::aquire_item),
          static_cast<void (Player::*)(const std::string &)>(
              &Player::aquire_item),
          static_cast<void (Player::*)(const Lua_item &)>(&Player::aquire_item),
          static_cast<void (Player::*)(const Item &)>(&Player::aquire_item)),
      "aquire_status",
      sol::overload(static_cast<void (Player::*)(int)>(&Player::aquire_status),
                    static_cast<void (Player::*)(const std::string &)>(
                        &Player::aquire_status),
                    static_cast<void (Player::*)(const Lua_status &)>(
                        &Player::aquire_status)),
      "remove_status",
      static_cast<void (Player::*)(int)>(&Player::remove_status),
      "remove_status",
      static_cast<void (Player::*)(const std::string &)>(
          &Player::remove_status),
      // make sure we get the correct overloaded take_damage function
      "take_damage", static_cast<void (Player::*)(int)>(&Player::take_damage),
      "turn_count", sol::property(&Player::turn_count), "die", &Player::die,
      "is_dead", &Player::is_dead, "pos", &Player::pos);
  lua_state.new_usertype<Monster>(
      "Monster",
      sol::constructors<Monster(int, int, const Pos &),
                        Monster(const std::string &, int, const Pos &),
                        Monster(const Lua_monster &, int, const Pos &)>(),
      "hp", sol::property(&Creature::get_hp), "attack",
      sol::property(&Creature::get_attack), "name",
      sol::property(&Monster::name), "do_attack", &Monster::do_attack,
  ...
  lua_state.new_usertype<Game>(
      ...
      "lua_manager",
      sol::property([](Game &self) { return std::ref(self.lua_manager); }),
      "send_msg", &Game::send_msg, "map",
      sol::property([](Game &self) { return std::ref(self.map()); }));
  lua_state.new_usertype<Lua_manager>(
      "Lua_manager", sol::no_constructor, "get_func", &Lua_manager::get_func,
      "get_item",
      static_cast<const Lua_item &(Lua_manager::*)(int) const>(
          &Lua_manager::get_item),
      "get_item",
      static_cast<const Lua_item &(Lua_manager::*)(const std::string &) const>(
          &Lua_manager::get_item),
      "get_mon",
      static_cast<const Lua_monster &(Lua_manager::*)(int) const>(
          &Lua_manager::get_mon),
      "get_mon",
      static_cast<const Lua_monster &(Lua_manager::*)(const std::string &)
                      const>(&Lua_manager::get_mon),
      "get_status",
      static_cast<const Lua_status &(Lua_manager::*)(int) const>(
          &Lua_manager::get_status),
      "get_status",
      static_cast<const Lua_status &(Lua_manager::*)(const std::string &)
                      const>(&Lua_manager::get_status),
      "get_rand_item", &Lua_manager::get_rand_item, "get_rand_mon",
      &Lua_manager::get_rand_mon);
  lua_state.new_usertype<Message>(
      "Message",
      sol::constructors<Message(), Message(const std::string &),
                        Message(const std::string &, const TCODColor &),
                        Message(const std::string &, const TCODColor &,
                                bool)>());
...
void Lua_manager::script(const std::string &input) { lua_state.script(input); }

std::optional<std::string>
Lua_manager::script_friendly(const std::string &input) {
  try {
    script(input);
  } catch (sol::error &err) {
    return err.what();
...
const Lua_item &Lua_manager::get_item(const std::string &name) const {
  return *std::find_if(
      item_generators.begin(), item_generators.end(),
      [&name](const Lua_item &item) { return item.name == name; });
}
...
const Lua_monster &Lua_manager::get_mon(const std::string &name) const {
  return *std::find_if(
      monster_generators.begin(), monster_generators.end(),
      [&name](const Lua_monster &monster) { return monster.name == name; });
}
...
const Lua_status &Lua_manager::get_status(const std::string &name) const {
  return *std::find_if(
      status_generators.begin(), status_generators.end(),
      [&name](const Lua_status &status) { return status.name == name; });
}
const Lua_item &Lua_manager::get_rand_item(int depth) const {
  auto range = item_rarity_map.equal_range(
      rand_int_log_bounded(depth - 2, depth + 2, 0, game->max_depth));
  return *random_element(range.first, range.second)->second;
}

const Lua_monster &Lua_manager::get_rand_mon(int depth) const {
  auto range = monster_rarity_map.equal_range(
      rand_int_log_bounded(depth - 2, depth + 2, 0, game->max_depth));
  return *random_element(range.first, range.second)->second;
}
