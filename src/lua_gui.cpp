...
Lua_gui::Lua_gui(Pos pos, int width, int height, std::function<void()> close_callback)
	: messages(pos.x, pos.y, width, height-2), window(Pos{pos.x, pos.y + height-3}, width, 3), close_callback(close_callback)
{
}
...
bool Lua_gui::handle_input(const TCOD_key_t &input) {
	if (input.vk == TCODK_ENTER) {
		...
		current_entry = "";
	 	...
	 	current_entry += input.text; // doesn't seem to work? never get TCODK_TEXT event
